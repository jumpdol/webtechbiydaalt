...
<script>
	function login() {
		...
		xml.onreadystatechange = function() {
			if (xml.readyState == 4 && xml.status == 200) {
				...
				if (xml.response == "1") {
					...
					window.location = "http://localhost/demo1/dist/index.php";
				} else {
					window.location = "http://localhost/demo1/dist/user.php";
				}
				var n = xml.response.search("нууц үг буруу")
				if (n != -1) {
					alert(xml.response)
					location.reload()
		...
		xml.open("POST", "http://localhost/demo1/dist/custom/pages/login/classic/loginAction.php", true);
		xml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xml.send("username=" + username + "&password=" + pass);
	}
	...
	function register() {
		...
		if (username != "" && fullname != "" && pass != "" && passr != "" && phone != "") {
			var xml = new XMLHttpRequest();
			xml.onreadystatechange = function() {
				if (xml.readyState == 4 && xml.status == 200) {
					if (xml.responseText == "suc") {
						var r = confirm("Та амжилттай бүртгэгдлээ одоо бүртглээ ашиглан нэвтрэнэ үү");
						if (r == true) {
							location.reload()
						} else {
							location.reload()
						...
						var n = xml.response.search("Duplicate entry")
						if (n != -1) {
							alert("Ижил хэрэглэгчийн нэртэй хэрэглэгч байна")
							location.reload()
			...
			xml.open("POST", "http://localhost/demo1/dist/custom/pages/login/classic/registerRes.php", true);
			xml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xml.send("username=" + username + "&fullname=" + fullname + "&pass=" + pass + "&passr=" + passr + "&phone=" + phone);
					...
					<div class="login-wrapper">
						...
						<div class="login-signin">
							...
							<form class="form text-left" id="kt_login_signin_form">
								<div class="form-group py-2 m-0">
									<input class="form-control h-auto border-0 px-0 placeholder-dark-75" id="logUsername" type="text" placeholder="Username" name="username" autocomplete="off" />
								</div>
								<div class="form-group py-2 border-top m-0">
									<input class="form-control h-auto border-0 px-0 placeholder-dark-75" id="logPass" type="Password" placeholder="Password" name="password" />
								</div>
								...
								<div class="text-center mt-15">
									<button id="kt_login_signin_submit" onclick="login()" class="btn btn-primary btn-pill shadow-sm py-4 px-9 font-weight-bold">Sign In</button>
								</div>
							</form>
						</div>
						<!--end:Sign In Form-->
						...
						<div class="login-signup">
							...
							<form class="form text-left" id="kt_login_signup_form">
								<div class="form-group py-2 m-0">
									<input id="fullname" class="form-control h-auto border-0 px-0 placeholder-dark-75" type="text" placeholder="Fullname" name="fullname" />
								</div>
								<div class="form-group py-2 m-0 border-top">
									<input id="username" class="form-control h-auto border-0 px-0 placeholder-dark-75" type="text" placeholder="Username" name="username" autocomplete="off" />
								</div>
								<div class="form-group py-2 m-0 border-top">
									<input id="pass" class="form-control h-auto border-0 px-0 placeholder-dark-75" type="password" placeholder="Password" name="password" />
								</div>
								<div class="form-group py-2 m-0 border-top">
									<input id="passr" class="form-control h-auto border-0 px-0 placeholder-dark-75" type="password" placeholder="Confirm Password" name="cpassword" />
								</div>
								<div class="form-group py-2 m-0 border-top">
									<input id="phone" class="form-control h-auto border-0 px-0 placeholder-dark-75" type="text" placeholder="Phone" name="phone" />
								</div>
								<div class="form-group d-flex flex-wrap flex-center">
									<button onclick="register()" id="kt_login_signup_submit" class="btn btn-primary btn-pill font-weight-bold px-9 py-4 my-3 mx-2">Submit</button>
